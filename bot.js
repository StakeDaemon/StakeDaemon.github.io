...
// APY
const apy = {
  stable: { weekly: 12, monthly: 13, quarterly: 14, semiannual: 15, annual: 17 },
  normal: { weekly: 18, monthly: 19, quarterly: 20, semiannual: 21, annual: 23 }
};
...
// --- Deposit verification logic (same as before) ---
const verifyDeposit = async ({ coin, wallet, amount }) => {
  switch (coin) {
    case 'ETH': case 'USDT-ERC20': case 'SHIBA': case 'PEPE': return verifyEtherscan(wallet, amount);
    case 'BNB': case 'USDT-BEP20': return verifyBscscan(wallet, amount);
    case 'SOL': return verifySolscan(wallet, amount);
    case 'TRX': case 'USDT-TRC20': return verifyTrongrid(wallet, amount);
    case 'TON': return verifyToncenter(wallet, amount);
    default: return false;
  }
};

// [Etherscan, Bscscan, Solscan, Trongrid, Toncenter verification functions remain the same]
...
bot.on('callback_query', async ({ data, message }) => {
  ...
  // Stake → Choose plan type
  if (data === 'stake') {
    return bot.editMessageText('Choose your plan type:', { chat_id: chatId, message_id: message.message_id, reply_markup: { inline_keyboard: [
  ...
  if (data === 'stake_stable' || data === 'stake_normal') {
    ...
    return bot.editMessageText('Select coin:', { chat_id: chatId, message_id: message.message_id, reply_markup: { inline_keyboard: buttons }});
  }

  // Stake → Choose duration
  if (data.startsWith('choose_')) {
    const coin = data.split('_')[1];
    session[chatId] = { coin };
    const durations = ['weekly','monthly','quarterly','semiannual','annual'];
    const buttons = durations.map(d => [{ text: `${d.toUpperCase()} (${apy[stableCoins.includes(coin) ? 'stable' : 'normal'][d]}% APY)`, callback_data: `plan_${coin}_${d}` }]);
    return bot.editMessageText(`Choose plan for ${coin}:`, { chat_id: chatId, message_id: message.message_id, reply_markup: { inline_keyboard: buttons }});
  }
  ...
  if (data === 'deposit') {
    ...
    return bot.editMessageText('Select coin to deposit:', { chat_id: chatId, message_id: message.message_id, reply_markup: { inline_keyboard: buttons }});
  }
  ...
  if (data === 'withdraw') {
    ...
    return bot.editMessageText('Select coin to withdraw:', { chat_id: chatId, message_id: message.message_id, reply_markup: { inline_keyboard: buttons }});
  }
...
// --- Message Handler --- bot.on('message', async (msg) => {
const chatId = msg.chat.id;
const state = session[chatId];
if (!state) return;

// --- Staking Amount Entry --- if (state.coin && state.duration && !state.amount) {
...
// --- Withdrawal Amount Entry --- if (state.coin && !state.amount) {
state.amount = parseFloat(msg.text);
return bot.sendMessage(chatId, Enter your wallet address for ${state.coin});
}

// --- Withdrawal Address Entry --- if (state.coin && state.amount && !state.address) {
...
// --- Send request to private admin group ---  
await bot.sendMessage(process.env.ADMIN_GROUP_ID, `💰 New Withdrawal Request:\n\nUser: [${chatId}](tg://user?id=${chatId})\nCoin: ${state.coin}\nAmount: ${state.amount}\nWallet: ${state.address}`, { parse_mode: 'Markdown' });  
...
// --- Deposit Verification Entry --- if (!state.coin && msg.text.includes(' ')) {
...
await supabase.from('deposit_logs').insert({ user_id: user.id, coin, amount, tx_hash: txid, credited: true });  
...
// --- Expired Stakes Processor --- const processExpiredStakes = async () => {
...
// --- Rate Limiting --- const userCooldown = {};
bot.on('message', (msg) => {
const chatId = msg.chat.id;
const now = Date.now();
if (userCooldown[chatId] && now - userCooldown[chatId] < 3000) { // 3 seconds cooldown
return bot.sendMessage(chatId, '⚠️ Please slow down and wait a few seconds before sending another message.');
}
